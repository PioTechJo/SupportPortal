    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
      <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
        <h3 className="font-semibold text-slate-800 flex items-center gap-2">
          ...
          {t('diagnosticBuilder.questions')}
        </h3>
        {categoryId && (
          <button onClick={openNew} title={t('diagnosticBuilder.newQuestion')} className="text-blue-600 hover:text-blue-800 p-1">
      ...
      <div className="overflow-y-auto flex-1 p-2 space-y-1">
        {!categoryId ? (
          <div className="p-4 text-center text-sm text-slate-500">{t('diagnosticBuilder.selectCategoryFirst')}</div>
        ) : loading ? (
          <div className="p-4 text-center text-sm text-slate-400 animate-pulse">{t('diagnosticBuilder.loading')}</div>
        ) : questions.length === 0 ? (
          <div className="p-4 text-center text-sm text-slate-500 bg-slate-50 rounded-lg border border-dashed border-slate-200">{t('diagnosticBuilder.questions')} - {t('diagnosticBuilder.loading')}</div>
        ) : (
          questions.map(q => (
            <div 
              key={q.id}
              ...
              <div className="flex-1 pe-2">
                ...
                <div className="text-xs text-slate-400 flex flex-wrap items-center gap-2 mt-1.5">
                  <span className="bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded uppercase text-[10px] font-bold">
                    {q.question_type === 'single_choice' ? t('diagnosticBuilder.singleChoice') : 'Text'}
                  </span>
                  <span>{t('diagnosticBuilder.displayOrder')}: {q.display_order}</span>
                  {q.question_type === 'single_choice' && (
                    <>
                      <span>•</span>
                      <span>{q.ai_question_options[0]?.count || 0} {t('diagnosticBuilder.options')}</span>
                    </>
                  )}
                </div>
              </div>
              <div className={`flex gap-1 ${selectedQuestionId === q.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
                <button onClick={(e) => openEdit(e, q)} title={t('diagnosticBuilder.edit')} className="p-1.5 text-slate-400 hover:text-blue-600 rounded">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </button>
                <button onClick={(e) => { e.stopPropagation(); handleDelete(q); }} title={t('diagnosticBuilder.delete')} className="p-1.5 text-slate-400 hover:text-red-600 rounded">
