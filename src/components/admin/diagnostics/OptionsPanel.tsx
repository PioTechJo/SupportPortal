  ...
  if (questionType === 'text') {
    return (
      <div className="bg-slate-50 rounded-xl border border-slate-200 flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <h3 className="font-semibold text-slate-700">{t('diagnosticBuilder.textInputQuestion')}</h3>
        <p className="text-sm text-slate-500 mt-2 max-w-xs">
          {t('diagnosticBuilder.textInputQuestionDesc')}
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
      <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
        <h3 className="font-semibold text-slate-800 flex items-center gap-2">
          ...
          {t('diagnosticBuilder.options')}
        </h3>
        {questionId && questionType === 'single_choice' && (
          <button onClick={openNew} title={t('diagnosticBuilder.newOption')} className="text-amber-600 hover:text-amber-800 p-1">
      ...
      <div className="overflow-y-auto flex-1 p-2 space-y-1">
        {!questionId ? (
          <div className="p-4 text-center text-sm text-slate-500">{t('diagnosticBuilder.selectQuestionFirst')}</div>
        ) : loading ? (
          <div className="p-4 text-center text-sm text-slate-400 animate-pulse">{t('diagnosticBuilder.loading')}</div>
        ) : options.length === 0 ? (
          <div className="p-4 text-center text-sm text-slate-500 bg-slate-50 rounded-lg border border-dashed border-slate-200">{t('diagnosticBuilder.noOptionsFound')}</div>
        ) : (
          options.map(opt => (
            <div 
              key={opt.id}
            ...
            >
              <div className="flex-1">
                ...
                <div className="text-xs text-slate-400 flex items-center gap-2 mt-1">
                  <span className="font-mono text-[10px] bg-slate-100 px-1 rounded">{t('diagnosticBuilder.val')}: {opt.option_value}</span>
                  <span>{t('diagnosticBuilder.displayOrder')}: {opt.display_order}</span>
                </div>
              </div>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={(e) => openEdit(e, opt)} title={t('diagnosticBuilder.edit')} className="p-1.5 text-slate-400 hover:text-amber-600 rounded">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </button>
                <button onClick={(e) => { e.stopPropagation(); handleDelete(opt); }} title={t('diagnosticBuilder.delete')} className="p-1.5 text-slate-400 hover:text-red-600 rounded">
