...
export const RulesList: React.FC<RulesListProps> = ({ categoryId, onEdit, triggerRefresh }) => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(t('recommendationRules.deleteConfirm'))) return;
  ...
  if (loading) return <div className="p-8 text-center animate-pulse text-slate-500">{t('recommendationRules.loadingRules')}</div>;
  
  if (rules.length === 0) {
    return (
      <div className="p-8 bg-slate-50 border border-slate-200 rounded-xl text-center text-slate-500">
        {t('recommendationRules.noRules')}
      </div>
    );
  }
      ...
      {rules.map((rule) => {
        ...
        return (
          <div key={rule.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm transition hover:shadow-md flex flex-col sm:flex-row relative">
            {isDuplicate && (
              <div 
                className="absolute top-3 end-3 text-amber-500"
                title={t('recommendationRules.duplicateWarning')}
              >
            ...
            <div className="flex-1 p-5 border-b sm:border-b-0 sm:border-e border-slate-100">
              <div className="flex items-center gap-2 mb-3">
                <span className="bg-teal-100 text-teal-800 text-xs font-bold px-2 py-1 rounded">
                  {t('recommendationRules.score')}: {rule.confidence_score}
                </span>
                <h4 className="font-semibold text-slate-800">{t('recommendationRules.matchConditions')}</h4>
              </div>
              
              <ul className="space-y-2">
                {criteriaEntries.length === 0 && <li className="text-sm text-slate-500 italic">{t('recommendationRules.noConditions')}</li>}
                {criteriaEntries.map(([qId, val], idx) => (
                  <li key={idx} className="text-sm flex gap-2">
                    <span className="font-medium text-slate-700 bg-slate-100 px-2 py-0.5 rounded">
                      {questionsMap[qId] || t('recommendationRules.unknownQuestion')}
                    </span>
            ...
            <div className="flex-1 p-5 flex flex-col justify-between bg-slate-50">
              <div className="space-y-4">
                <div>
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{t('recommendationRules.recommendation')}</h4>
                  <p className="text-sm text-slate-800">{rule.recommendation_text}</p>
                </div>
                {rule.root_cause_text && (
                  <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{t('recommendationRules.rootCause')}</h4>
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-slate-200">
                ...
                >
                  {t('recommendationRules.editRule')}
                </button>
                <button
                  ...
                  title={t('recommendationRules.delete')}
                >
                  {t('recommendationRules.delete')}
                </button>
              </div>
            </div>
          </div>
