...
export const EditOrganizationModal: React.FC<EditOrganizationModalProps> = ({
  ...
  const [code, setCode] = useState(initialData.domain?.replace('.com', '').toUpperCase() || '');
  const [domain, setDomain] = useState(initialData.domain || '');
  const [country, setCountry] = useState(initialData.country || '');
  const [tier, setTier] = useState<'standard' | 'premium' | 'enterprise'>(initialData.support_tier || 'standard');
  const [color, setColor] = useState(initialData.primary_color || '#0f766e');
  const [logo, setLogo] = useState(initialData.logo_url || '🏢');
  const [description, setDescription] = useState('');
  const [status, setStatus] = useState((initialData as any).status || 'active');
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <form id="edit-org-form" onSubmit={handleSubmit} className="space-y-6">
            {error && (
