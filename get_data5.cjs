...
async function run() {
  const email = `testuser_${Date.now()}@piotech.com`;
  ...
  // Create a record in users table because auth.users is separate from public.users usually,
  // but if there's a trigger it might be created automatically.
  // Wait, let's wait a second in case there's a trigger.
