-- Drop and recreate as SECURITY DEFINER with manual role-based filtering
-- This bypasses RLS per-row overhead (which caused 57014 timeout on 4,404 rows)
-- Data scoping is handled manually inside the function using auth.uid()
...
BEGIN
  ...
  -- Look up role and customer from users table (one lookup, not per-row)
  ...
  -- Internal roles: ADMIN, SYS_ADMIN, SUPPORT_ENGINEER, SUPPORT_MANAGER, CEO, TEAM_LEAD, TEAM_MEMBER, AGENT
  ...
  RETURN (
    WITH filtered_tickets AS (
      ...
      WHERE t.created_at::date >= p_from_date
        ...
        -- Role-based data scoping (replaces RLS)
