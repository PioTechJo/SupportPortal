-- Migration: Show legacy (pre-account) assignees in Engineer Performance.
-- Migrated tickets have no assigned_user_id yet (no real account created),
-- but do carry a legacy_assigned_to text name from the import. Fold that
-- name into the engineer performance breakdown so management can see the
-- current workload distribution before real accounts exist.
