-- Migration: Fix Engineer Performance to group by the column the app
-- actually writes to (tickets.assigned_to), not the unused/legacy
-- tickets.assigned_user_id column. The two columns both exist on tickets
-- (schema drift from earlier migrations) but only assigned_to is kept in
-- sync by ticket creation/assignment code, so grouping by assigned_user_id
-- silently produced wrong "engineer" ids and made click-through to the
-- ticket list impossible to wire up correctly.
...
BEGIN
  ...
  RETURN (
    ...
    engineer_performance AS (
      ...
      FROM filtered_tickets
      GROUP BY assigned_to, assigned_to_name, legacy_assigned_to
    ),
    escalations AS (
      SELECT
