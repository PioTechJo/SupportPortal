-- Migration: Fix duplicate rows in Engineer Performance / engineer filter.
-- The previous version grouped by (assigned_to, assigned_to_name, legacy_assigned_to)
-- together. Now that real accounts are linked via assigned_to, the same person
-- has multiple legacy_assigned_to spelling variants (e.g. "Firyal Mithalouni" vs
-- "Feryal Mithalouni") across their old tickets, which split them into duplicate
-- rows even though assigned_to is identical. Group by the resolved identity only.
