-- Add a "management_escalated_tickets" count to the dashboard analytics RPC,
-- so the Overview page can show a card for tickets escalated straight to
-- management by the bank (separate from internal team escalations).
  ...
  RETURN (
    WITH filtered_tickets AS (
      ...
      WHERE t.created_at::date >= p_from_date
        ...
        AND (p_engineer_ids IS NULL OR t.assigned_to = ANY(p_engineer_ids))
    ),
    metrics AS (
      SELECT
        ...
        count(*) FILTER (WHERE status_code = 'DEVELOPMENT_ACTION') AS development_action_tickets,
        count(*) FILTER (WHERE status_code IN ('CLOSED', 'APPROVED')) AS closed_tickets,
        count(*) FILTER (WHERE is_management_escalated = true) AS management_escalated_tickets
    ...
    zero_ticket_banks AS (
      SELECT c.id, c.customer_name AS name, c.country
      FROM public.customers c
      WHERE v_is_internal
        AND COALESCE(c.is_internal, false) = false
        AND NOT EXISTS (
          SELECT 1 FROM filtered_tickets ft WHERE ft.customer_id = c.id
        )
    ...
    engineer_performance AS (
      SELECT
        COALESCE(assigned_to::text, 'legacy:' || legacy_assigned_to, 'unassigned') AS id,
        COALESCE(MAX(assigned_to_name), MAX(legacy_assigned_to), 'Unassigned') AS name,
        count(*) AS assigned,
      ...
      FROM filtered_tickets
      GROUP BY COALESCE(assigned_to::text, 'legacy:' || legacy_assigned_to, 'unassigned')
    ),
    escalations AS (
      SELECT
    ...
    SELECT jsonb_build_object(
      ...
      'zeroTicketBanks', (SELECT COALESCE(jsonb_agg(row_to_json(z)), '[]'::jsonb) FROM zero_ticket_banks z),
